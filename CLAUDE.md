...
- `pans/` —— 四类业务页 + 登录页。
  ...
  - `childs/`：`customcard`(排班格子，通用可复用)、`selbtn`(今天/本周/本月分段按钮)、`medicalcardwidget`(预约卡)、`mannger/doctorcard.{h,cpp}`(DoctorSlotCard：扁时段瓦片，管理员排班用)、`mannger/copyschedule.{h,cpp}`(CopyScheduleDialog：批量复制排班弹窗，管理员排班页 DoctorOrder 专用；选 N 周+日期显示+确认/取消+二级确认，确认后由 DoctorOrder 发包)、`mannger/smartschedule.{h,cpp}`(SmartScheduleDialog：智能排班弹窗，模式/禁排矩阵/预览/应用；与 smartplan 配合，进入不联网)、`mannger/smartplan.{h,cpp}`(纯算法模块，医生序号空间：`SmartPlanIn/Out`、`runPlan()` 按算法 id 分发、`planBalancedWeek()` 本周尽量平均)。`chatbom`、`circularavatar` 已编译但**从未被实例化（遗留死代码）**。
